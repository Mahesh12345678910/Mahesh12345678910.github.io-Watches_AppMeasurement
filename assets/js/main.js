...
this.onclick=function(e){
    ...
    if(parentEle)
    {
        if(e.target.className=="bx bx-minus")
        {
            let nextSibling=parentEle.nextElementSibling;
            let itemval=parseInt(nextSibling.innerText);
            itemval-=1;
            nextSibling.innerText=""+itemval;

        }
        if(e.target.className=="bx bx-plus")
        {
           
            let nextSibling=parentEle.previousElementSibling;
            let itemval=parseInt(nextSibling.innerText);
            itemval+=1;
            nextSibling.innerText=""+itemval;
...
{
    let iprod=prods[i];
    iprod.addEventListener("click",function(e){
        
        console.log(e.target);
        let children=e.target.children;
        let imgsrc=children[1].src;

        let prod_Class=children[1].className;
        
        let prod_Category=prod_Class.split("__")[0];
        console.log(prod_Category);

        let imgdata=children[2].children;
        let imgtitle=imgdata[0].innerText;
        let imgprice=imgdata[1].innerText;
        console.log(imgsrc,imgprice,imgtitle);

        localStorage.setItem("prod_price",imgprice);
